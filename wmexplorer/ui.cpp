...
intptr_t far_message(const GUID& guid, const UnicodeString& msg, intptr_t button_cnt, FARMESSAGEFLAGS flags) {
...
ObjectArray<UnicodeString> get_size_suffixes() {
  ObjectArray<UnicodeString> suffixes;
...
void draw_copy_files_progress(const CopyFilesProgress& progress, const CopyFilesStats& stats, bool move) {
  ...
  // paths
  UnicodeString src_label(far_get_msg(MSG_COPY_FILES_PROGRESS_SRC));
  ...
  lines += UnicodeString::format(far_get_msg(MSG_COPY_FILES_PROGRESS_FILE_SIZE).data(), &format_data_size(progress.copied_file_size, get_size_suffixes()), &format_data_size(progress_file_size, get_size_suffixes()), file_percent, &format_data_size(file_speed, get_speed_suffixes()));
  // file progress bar
  unsigned len1;
...
bool verify_hot_key(HANDLE h_dlg, int ctrl_id) {
  UnicodeString hot_key = far_get_text(h_dlg, ctrl_id);
...
intptr_t WINAPI plugin_options_dlg_proc(HANDLE hDlg, intptr_t Msg, intptr_t Param1, void* Param2) {
  ...
  if (Msg == DN_INITDIALOG) {
    ...
    UnicodeString prefix = far_get_text(hDlg, dlg_data->prefix_ctrl_id);
    if (prefix.size() == 0) {
    ...
    if (auto_buf_size) options->copy_buf_size = -1;
      ...
      UnicodeString bs = far_get_text(hDlg, dlg_data->copy_buf_size_ctrl_id);
...
bool show_filters_dlg(const ObjectArray<FileFilters>& filter_list, Array<FilterSelection>& filter_selection) {
  ...
  for (unsigned i = 0; i < filter_list.size(); i++) {
    // find hot key
    UnicodeString ext = filter_list[i].src_ext;
