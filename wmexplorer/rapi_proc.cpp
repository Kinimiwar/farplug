...
void overwrite_decision(const UnicodeString& dst_path, const FileInfo& src_file_info, const FileInfo& dst_file_info, bool& overwrite, OverwriteOption& ov_option) {
...
void list_add_dir(const UnicodeString& src_dir, WIN32_FIND_DATAW& src_find_data, CreateListStats& stats, CreateListOptions& options, UiLink& ui, Log& log, PluginInstance* plugin, FileList& file_list) {
  bool ignore_errors = options.ignore_errors;
  try {
    UnicodeString src_path = COMPOSE_PATH(src_dir, src_find_data.cFileName);
    bool more = true;
...
FileList create_file_list(const UnicodeString& src_dir, const UnicodeString& src_file_name, CreateListStats& stats, CreateListOptions& options, UiLink& ui, Log& log, PluginInstance* plugin) {
  ...
  bool ignore_errors = options.ignore_errors;
  try {
    UnicodeString src_path = COMPOSE_PATH(src_dir, src_file_name);
    WIN32_FIND_DATAW find_data;
    HANDLE h_find;
    plugin->last_object = src_path;
    BEGIN_RETRY_BLOCK;
...
void list_add_dir(const UnicodeString& src_dir, CE_FIND_DATA& src_find_data, CreateListStats& stats, CreateListOptions& options, UiLink& ui, Log& log, IRAPISession* session, PluginInstance* plugin, FileList& file_list) {
  bool ignore_errors = options.ignore_errors;
  try {
    UnicodeString src_path = COMPOSE_PATH(src_dir, src_find_data.cFileName);
    DWORD file_count;
...
void prepare_target_path(const UnicodeString& path, PluginInstance* plugin) {
  ...
  for (unsigned i = 1; i <= fp_path.size(); i++) {
    UnicodeString partial_path = fp_path.get_partial_path(i);
...
void copy_files(bool src_remote, const UnicodeString& src_dir, const FileList& src_file_list, bool dst_remote, const UnicodeString& dst_dir, const UnicodeString& dst_new_name, CopyFilesStats& stats, CopyFilesProgress& progress, CopyFilesOptions& options, UiLink& ui, AutoBuffer& buffer, Log& log, ObjectArray<FilterInterface>& filters, IRAPISession* session, PluginInstance* plugin) {
  ...
  UnicodeString src_path, dst_path;
  UnicodeString src_file_name, dst_file_name;
...
void create_dir(const UnicodeString& dst_dir, const UnicodeString& dst_file_name, IRAPISession* session, PluginInstance* plugin) {
  ...
  for (unsigned i = 1; i <= fp_dst_path.size(); i++) {
    UnicodeString partial_path = fp_dst_path.get_partial_path(i);
    if (!dir_exists(partial_path, session)) {
      plugin->last_object = partial_path;
      CHECK_RAPI(RAPI(CeCreateDirectory(partial_path.data(), NULL)) != 0);
    }
...
void refresh_system_info(PluginInstance* plugin) {
  ...
  if (RAPI(CeGetVersionEx(&os_ver)) != 0) {
    ...
    sys_info.add_info(far_get_msg(MSG_SYSINFO_OS_VERSION), UnicodeString::format(L"%u.%u.%u", os_ver.dwMajorVersion, os_ver.dwMinorVersion, os_ver.dwBuildNumber).data());
  }
  ...
  DWORD file_count;
  CE_FIND_DATA* find_data;
  ObjectArray<UnicodeString> mount_points;
  ...
  if (RAPI(CeGetSystemPowerStatusEx(&power_status, TRUE)) != 0) {
    if (power_status.ACLineStatus != AC_LINE_UNKNOWN) {
      ...
      if (power_status.BatteryLifePercent != BATTERY_PERCENTAGE_UNKNOWN) {
        sys_info.add_info(far_get_msg(MSG_SYSINFO_POWER_BATTERY_LEVEL), UnicodeString::format(L"%u%%", power_status.BatteryLifePercent).data());
      }
      if (power_status.BackupBatteryLifePercent != BATTERY_PERCENTAGE_UNKNOWN) {
        sys_info.add_info(far_get_msg(MSG_SYSINFO_POWER_BACKUP_BATTERY_LEVEL), UnicodeString::format(L"%u%%", power_status.BackupBatteryLifePercent).data());
      }
    }
  }
}
...
UnicodeString get_open_command(const UnicodeString& file_dir, const UnicodeString& file_name, IRAPISession* session) {
  ...
  UnicodeString file_ext = file_name.right(file_name.size() - pos);
  // open HKEY_CLASSES_ROOT\.ext key and read File Type ID
  UnicodeString file_type_id;
  HKEY h_key;
