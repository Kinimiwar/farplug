...
ObjectArray<FileFilters> export_filter_list;
ObjectArray<FileFilters> import_filter_list;
...
ICeFileFilter* create_interface(const UnicodeString& guid_str) {
  ICeFileFilter* itf;
  GUID guid;
...
FilterInterface::FilterInterface(const UnicodeString& src_ext, const UnicodeString& dst_ext, const UnicodeString& guid_str): src_ext(src_ext), dst_ext(dst_ext), itf(create_interface(guid_str)) {
}
...
public:
  FileStream(const UnicodeString& file_name, bool read_only): file_name(file_name), read_only(read_only), h_file(INVALID_HANDLE_VALUE), ref_cnt(1) {
  }
  ~FileStream() {
    if (h_file != INVALID_HANDLE_VALUE) {
      ::CloseHandle(h_file);
...
private:
  UnicodeString src_file_name;
  UnicodeString dst_file_name;
public:
  FileFilterSite(UnicodeString src_file_name, UnicodeString dst_file_name): src_file_name(src_file_name), dst_file_name(dst_file_name) {
  }
  // IUnknown
  STDMETHODIMP QueryInterface(THIS_ REFIID riid, LPVOID *ppvObj) {
...
FilePathParts split_path(const UnicodeString& path) {
  FilePathParts parts;
