...
class DefragProgress: public ProgressMonitor {
protected:
  virtual void do_update_ui() {
    ...
    if (total_clusters != 0) {
      lines += UnicodeString::format(far_get_msg(MSG_DEFRAG_PROGRESS_EXTENTS).data(), extents_before, extents_after);
      lines += UnicodeString::format(far_get_msg(MSG_DEFRAG_PROGRESS_CLUSTERS).data(), moved_clusters, total_clusters);
...
void defragment(const ObjectArray<UnicodeString>& file_list, Log& log) {
  ...
  for (unsigned i = 0; i < file_list.size(); i++) {
    try {
      ...
      if (file_list.size() == 1) throw;
      log.add(lotDefragment, file_list[i], extract_file_name(oem_to_unicode(e.file)) + L":" + int_to_str(e.line) + L" " + e.message());
    }
  }
}
